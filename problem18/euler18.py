...
def main():
    """Dynamic prog: bottom up
       We choose max(63+4, 63+62), max(66+62, 66+98) and so on, and we get a new line of max.
       We iterate with the next line until we reach the top.
    """
    ...
    for i in range(n,1,-1):
            cur_row = [max(rows[i-2][j]+rows[i-1][j], rows[i-2][j]+rows[i-1][j+1]) for j in range(len(rows[i-2]))]
            rows[i-2] = cur_row
